...
| 目标 | 注释要求 | 说明 |
|------|----------|------|
| 所有类 / 接口 | 类注释 | 含 DTO、VO、Config、Entity、Enum、Utils、常量类、测试类，**一个都不能漏** |
| 所有变量 / 字段 | 字段注释 | 含 DTO/VO/Config 字段、private 字段、static 常量、测试数据变量 |
...
- [ ] 每个类（含 DTO/VO/Config/Enum/Utils/测试类）有类注释，写清「这个类做什么」
- [ ] 每个变量/字段（含 DTO/VO/Config 字段、常量）有注释，写清业务含义与约束
